...
const OrderChart = () => {
    ...
    const [locationType, setLocationType] = useState('bang'); // 'bang' or 'thanhpho'
            ...
            const transformedData = response.data.map(item => {
                ...
                switch (period) {
                    case 'nam':
                        ...
                        break;
                    case 'quy':
                        name = `Quý ${item.quy} - ${item.nam}`;
                        break;
                    case 'thang':
                        name = `Tháng ${item.thang} - ${item.nam}`;
                        break;
                    case 'ngay':
                        name = `${item.ngay}/${item.thang}/${item.nam}`;
                        break;
                    default:
                        name = `Năm ${item.nam}`;
                }
    ...
    const getLocationChartTitle = () => {
        return locationType === 'bang' 
            ? 'Biểu Đồ Tổng Tiền Theo Bang'
            : 'Biểu Đồ Tổng Tiền Theo Thành Phố';
    };
        ...
        <div className="container mt-4">
            ...
            <Row>
                ...
                <Col md={12}>
                    ...
                    <h3 className="text-center mb-3">{getLocationChartTitle()}</h3>
                    <div style={{ width: '100%', height: 400 }}>
                        <ResponsiveContainer>
                            <PieChart>
                                <Pie
                                    ...
                                    labelLine={true}
                                    label={({ name, percent }) => {
                                        if (percent < 0.05) return ''; // Không hiển thị nhãn cho phần quá nhỏ
